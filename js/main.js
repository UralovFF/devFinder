...
let mainForm = document.querySelector(".main-form__body");
let modeText = document.querySelector(".mode__text");
let modeIconSvg = document.querySelector(".mode > svg");
let modeIconPath = document.querySelector(".mode > svg > g > path");
let hoveredMode = document.querySelector('.hovered__mode')
...
let profileStatus = document.querySelector(".profile_status");
let accountStatus = document.querySelector(".account_status");
let accountStatusTitle = document.querySelectorAll(
  ".account__status__card__title"
);
let accountStatusText = document.querySelectorAll(
  ".account__status__card__text"
);
let mainFormNavs = document.querySelectorAll(".main-form__navs > a > p");
let mainFormNavsIcons = document.querySelectorAll(
  ".main-form__navs > a > svg > g > path"
);


mode.addEventListener("click", () => {
  body.classList.toggle("dark__bg");
  logo.classList.toggle("light__text");
  mainForm.classList.toggle("main-form__dark");
  searchBody.classList.toggle("search__body__dark");
  searchInput.classList.toggle("search__input__dark");
  nick.classList.toggle("light__text");
  date.classList.toggle("light__text");
  profileStatus.classList.toggle("light__text");
  accountStatus.classList.toggle("dark__bg");
  accountStatusText.forEach((item) => item.classList.toggle("light__text"));
  accountStatusTitle.forEach((item) => item.classList.toggle("light__text"));
  mainFormNavs.forEach((item) => item.classList.toggle("light__text"));
  mainFormNavsIcons.forEach((item) => item.classList.toggle("light__svg"));

  if (modeText.innerText == "Dark".toUpperCase()) {
    modeText.innerText = "Light";
    modeText.style.color = "#fff";
    modeIconSvg.innerHTML = `
            <g id="002-sun">
    ...
    `;
    modeIconPath.forEach(item => item.classList.toggle('hovered__mode'))
  } else {
...
mode.addEventListener('mouseover', () => {
    if (modeText.innerText == "Light".toUpperCase()){
        modeText.style.color = "#90a4d4"
        hoveredMode.style.fill = '#90a4d4'
    } else{
      modeIconPath.style.fill = '#000'
    }
})
...
mode.addEventListener('mouseout', () => {
    if (modeText.innerText == "Light".toUpperCase()){
      modeText.style.color = "#fff"
      hoveredMode.style.fill = ''
    } else{
      modeIconPath.style.fill = ''
    }
})